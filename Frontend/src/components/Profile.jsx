    ...
    */
    ...
    return (
        <div className="bg-white rounded-4xl shadow-xl p-8 w-full max-w-sm border border-gray-50 text-center relative mb-6">
            ...
            <div className="relative w-24 h-24 mx-auto mb-4">
                <span onClick={() => isOwnProfile && setIsSelectingPfp(!isSelectingPfp)} aria-label="Edit profile avatar"
                    className={`w-full h-full bg-green-100 text-green-600 rounded-full select-none flex items-center justify-center text-5xl shadow-inner border-4 border-white ${isOwnProfile && !isDoctor ? 'clickHover hover:bg-green-200' : ''}`}>
                    {pfp}
                </span>
                    ...
                    {pendingRequests.map((senderName) => (
                        <div key={senderName} className="bg-blue-50 border border-blue-100 p-3 rounded-xl flex items-center justify-between shadow-sm">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => onAcceptRequest && onAcceptRequest(senderName)} aria-label={`Accept friend request from ${senderName}`} className="bg-blue-500 text-white px-3 py-1 rounded-lg text-xs font-bold clickHover hover:bg-blue-600">
                                    ✓ Accept
                                </button>
                                <button onClick={() => onDeclineRequest && onDeclineRequest(senderName)} aria-label={`Decline friend request from ${senderName}`} className="bg-red-100 text-red-600 px-3 py-1 rounded-lg text-xs font-bold clickHover hover:bg-red-200">
                                    ✕
                                </button>
                            </div>
                        </div>
                ...
                <div className="space-y-2">
                    {pendingDoctorRequests.map((senderName) => (
                        <div key={senderName} className="bg-emerald-50 border border-emerald-100 p-3 rounded-xl flex items-center justify-between shadow-sm">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => onAcceptDoctorRequest && onAcceptDoctorRequest(senderName)} aria-label={`Accept doctor request from ${senderName}`} className="bg-emerald-500 text-white px-3 py-1 rounded-lg text-xs font-bold clickHover hover:bg-emerald-600">
                                    ✓ Accept
                                </button>
                                <button onClick={() => onDeclineDoctorRequest && onDeclineDoctorRequest(senderName)} aria-label={`Decline doctor request from ${senderName}`} className="bg-red-100 text-red-600 px-3 py-1 rounded-lg text-xs font-bold clickHover hover:bg-red-200">
                                    ✕
                                </button>
                            </div>
                        </div>
                    ...
                    <div className="relative flex flex-row items-center w-full">
                        <p className="text-lg uppercase font-black text-green-400 mb-1 mx-auto">
                            Friends
                                ...
                                <span className="absolute right-0 text-lg font-black text-green-200 mb-1 py-2 px-4 bg-green-400 rounded-full clickHover hover:bg-green-300 select-none" aria-label={addingFriend ? 'Close friend search' : 'Open friend search'} onClick={() => setAddingFriend(!addingFriend)}> 
                           {addingFriend ? "x" : "+"}
                        </span>
                    </div>
                    ...
                    {friendsList.length > 0 && ( // Only show list if the user has friends
                        <div className="flex flex-col gap-2 justify-center mt-3 pt-3 border-t border-green-200/50">
                            {friendsData.map((friend) => (
                                <button key={friend.username} onClick={() => onViewProfile && onViewProfile(friend.username)} aria-label={`View profile for ${friend.username}`} className="w-full p-3 bg-cyan-100 rounded-xl flex items-center justify-between shadow-sm clickHover-[1.05] hover:bg-cyan-200">
        ...
        {!isOwnProfile && isDoctor && assignedChecked && isAssignedUser && (
            <div className="bg-green-100 p-4 rounded-2xl w-full border border-green-100 shadow-sm">
                ...
                <div className="mb-4 mt-2 px-5 text-center">
                    <button onClick={createTask} aria-label="Assign a new task" className="bg-white px-6 py-4 text-2xl clickHover rounded-lg hover:bg-gray-200 m-2">
                        Assign Task
                    </button>
                </div>
            </div>
        ...
        {!isOwnProfile && friendshipChecked && !isAlreadyFriend && profileEmail !== loggedInUserEmail && !isDoctor && (
            <button onClick={handleSendViewedProfileRequest} aria-label="Send friend request" className="w-full py-3 bg-green-50 text-green-600 hover:bg-green-100 font-black rounded-2xl border border-green-100 clickHover mb-8">
                + Add Friend
            </button>
        )}
        ...
        {!isOwnProfile && friendshipChecked && isAlreadyFriend && profileEmail !== loggedInUserEmail && !isDoctor && (
            <button onClick={() => onRemoveFriend && onRemoveFriend(username)} aria-label={`Remove ${username} from friends`} className="w-full py-3 bg-red-100 text-red-500 hover:bg-red-200 font-black rounded-2xl border border-green-100 clickHover mb-8">
                - Remove Friend
            </button>
        )}
        ...
        {!isOwnProfile && profileEmail !== loggedInUserEmail && isDoctor && assignedChecked && !isAssignedUser && (
            <button onClick={handleSendDoctorAssignmentRequest} aria-label="Send doctor connection request" className="w-full py-3 bg-green-50 text-green-600 hover:bg-green-100 font-black rounded-2xl border border-green-100 clickHover mb-8">
                + Connect with user
            </button>
        )}
        ...
        {!isOwnProfile && assignedChecked && isAssignedUser && profileEmail !== loggedInUserEmail && isDoctor && (
            <button onClick={handleRemoveAssignedUser} aria-label={`Remove ${username} as assigned user`} className="w-full py-3 bg-red-100 text-red-500 hover:bg-red-200 font-black rounded-2xl border border-green-100 clickHover my-8">
                - Remove User
            </button>
        )}
            ...
            <div className="w-full mb-8">
                <div className="bg-gray-50 p-4 rounded-2xl w-full border border-gray-100">
                    ...
                    <div className="flex gap-2 mb-2">
                        ...
                        <button onClick={handleSearchUser} aria-label="Search for friend" className="bg-gray-800 text-white px-4 py-2 rounded-xl text-sm font-bold clickHover">
                            Search
                        </button>
                    </div>
                    ...
                    {searchResult && (
                        <div className="mt-3 p-3 bg-white rounded-xl border border-gray-200 flex items-center justify-between shadow-sm clickHover hover:bg-gray-50" aria-label={`View profile for ${searchResult.username}`} onClick={() => onViewProfile && onViewProfile(searchResult.username)}>
                            <div className="flex items-center gap-3">
            ...
            <div className="w-full mb-8">
                <div className="bg-gray-50 p-4 rounded-2xl w-full border border-gray-100">
                    ...
                    <div className="flex gap-2 mb-2">
                        ...
                        <button onClick={() => handleSearchUser()} aria-label="Search for user" className="bg-gray-800 text-white px-4 py-2 rounded-xl text-sm font-bold clickHover">
                            Search
                        </button>
                    </div>
        ...
        {isOwnProfile && (
            <button onClick={openEditDetailsMenu} aria-label="Edit profile details" className="w-full py-3 bg-cyan-100 text-cyan-500 hover:bg-cyan-200 font-black rounded-2xl border border-blue-100 clickHover-[1.05] mb-4">
                Edit Details
            </button>
        )}
        ...
        {isOwnProfile && (
            <button onClick={() => onSignOut && onSignOut()} aria-label="Sign out" className="w-full py-3 bg-red-100 text-red-500 hover:bg-red-200 font-black rounded-2xl border border-red-100 clickHover-[1.05]">
                Sign Out
            </button>
        )}
    </div>
