...
const Item = ({name, icon: Icon, path}) => {
    return (
        <Link to={path} aria-label={`Go to ${name}`}>
            <div className="w-20 flex flex-col items-center text-green-600 clickHover-[1.15]">
            ...
            <>
               {/* Left component : Assigned users */}
                ...
                <>
                    ...
                    <button type="button" onClick={handleStartStop} aria-label={isRecording ? 'Stop walk recording' : 'Start walk recording'} className={`w-20 h-20 rounded-full flex items-center justify-center shadow-xl border-4 active:scale-95 clickHover ${isRecording ? 'bg-red-500 border-red-700' : 'bg-white border-gray-600'}`}>
